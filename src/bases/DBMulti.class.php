...
class DBMulti
{
	...
	function selectDb($dbName, $dbConn=null){
        ...
        //重新选择当前链接所选数据库
        if ($this->dbCurrConn && is_resource($this->dbCurrConn)){
            if (!mysql_select_db($dbName, $this->dbCurrConn)){
                $this->errorLog("Select database: $dbName failed.", $this->dbCurrConn);
                $status = false;
            }
        }
		//选择Master数据库
	...
	function errorLog($msg='', $conn=null){
		...
		$log .= "  [$page_name - ". date("Y-m-d H:i:s") ."]";
		error_log($log);
		return true;
	}
...
//        return true;
     ...
     * 获取最后一次数据库连接信息
     *
     * @return resource 返回最后一次数据库连接
     */
    function getLastConn(){
        return $this->dbCurrConn;
    }

    /**
     * 获取最后一次数据库查询错误
     *
     * @return string 返回最后一次查询错误
     */
    function getLastError(){
        if (!($lastConn = $this->getLastConn())){
            return false;
        }
        $err = mysql_errno($lastConn) .":". mysql_error($lastConn);
        return $err;
    }
